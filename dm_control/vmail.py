...
class VMAIL(tools.Module):
  ...
  def _train(self, model_data, expert_data, log_images):
    with tf.GradientTape() as model_tape:
      ...
      # 不变特征提取 transition loss 
      prior_dist = self._dynamics.get_dist(prior)
      post_dist = self._dynamics.get_dist(post)
      div = tf.reduce_mean(tfd.kl_divergence(post_dist, prior_dist))
      div = tf.maximum(div, self._c.free_nats)

      # 背景特征提取 transition loss 
    ...
    with tf.GradientTape() as mi_tape:
      ...
      train_mi = self._mi_est.train(post_expert1['stoch'],post_expert2['stoch'])
      mi_loss = self._mi_est.get_mi_loss(post_expert1['stoch'],post_expert2['stoch'])
    ...
    if tf.distribute.get_replica_context().replica_id_in_sync_group == 0:
      if self._c.log_scalars:
        self._scalar_summaries(
            model_data, feat, prior_dist, post_dist, likes, div, bg_div, model_loss,mi_loss,train_mi[-1],
            ...
            value_loss, actor_loss, model_norm, mi_norm, discriminator_norm, value_norm, actor_norm)
  ...
  def _build_model(self):
    ...
    self._mi_est = models.MIEstimator()
    ...
    mi_modules = [self._encode,self._dynamics]
    if self._c.pcont:
    ...
    self._mi_opt = Optimizer('mi', mi_modules, self._c.mi_lr)
  ...
  def _scalar_summaries(
      self, data, feat, prior_dist, post_dist, likes, div, bg_div, model_loss,mi_loss,train_mi,
...
def make_env(config, writer, prefix, model_datadir, policy_datadir, store):
  ...
  print(task)
  if suite == 'dmc':
    env = wrappers.DeepMindControl(task,img_source='video',resource_files='video/idealgas0.mp4')
...
def main(config):
  ...
  expert_datadir = os.path.join(config.expert_datadir,task+'_expert2')
